...
export class MCPServer {
    ...
    public static async create(command: string, args: string[], timeout: number = DEFAULT_TIMEOUT) {
        ...
        transport.onclose = () => console.debug('MCP transport closed');
        ...
        return new MCPServer(client, timeout);
    }

    public async listTools(): Promise<Tool[]> {
        const toolsResult = await this.client.listTools(undefined, { timeout: this.timeout });
        return toolsResult.tools;
    }

    public async callTool(name: string, parameters: any): Promise<CallToolResult> {
        return this.client.callTool({ name, arguments: parameters }, CallToolResultSchema, { timeout: this.timeout });
    }
}
