...
import { initializeDatabase, setupDbHandlers, getUserSettings } from "./main/db";
...
// Set up logging
const setupLogging = (debug = false) => {
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const enableAnalytics =
      process.env.ENABLE_ANALYTICS?.toLowerCase() === "true" || false;
    ...
    // Now that MCP is ready, initialize LLM providers
    const tools = await listTools();
    const anthropicService = new AnthropicService(tools, analytics);
    const openaiService = new OpenAIService(tools, analytics);

    // Create and configure the LLM service provider
